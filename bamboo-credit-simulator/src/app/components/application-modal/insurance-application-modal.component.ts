              ...
              <div class="quote-summary">
                ...
                <div class="summary-grid">
                  <div class="summary-item">
                    ...
                    <span class="value">{{ quoteData?.productName || productData?.name }}</span>
                  </div>
                  <div class="summary-item">
                  ...
                  <div class="summary-item">
                    ...
                    <span class="value">{{ formatCurrency(quoteData?.monthlyPremium || 0) }}</span>
                  </div>
                  <div class="summary-item">
                    <span class="label">Prime annuelle:</span>
                    <span class="value">{{ formatCurrency(quoteData?.annualPremium || 0) }}</span>
                  </div>
                  <div class="summary-item">
                    <span class="label">Franchise:</span>
                    <span class="value">{{ formatCurrency(quoteData?.deductible || 0) }}</span>
                  </div>
                  <div class="summary-item">
                    <span class="label">Couverture:</span>
                    <span class="value">{{ formatCurrency(quoteData?.coverageAmount || 0) }}</span>
                  </div>
                </div>
              </div>
            </div>
...
async onSubmit(): Promise<void> {
  ...
  try {
    ...
    // ⚠️ CORRECTION ICI : Utiliser l'ID correct du produit
    let validProductId = this.productData?.id || '';
    
    // Si l'ID n'est pas valide, utiliser un ID par défaut selon le type d'assurance
    const insuranceType = this.getInsuranceType();
    
    if (!validProductId || validProductId === 'main_offer') {
      // Mapper les types d'assurance vers des IDs valides
      const productIdMap = {
        'auto': 'ogar_auto_tr',           // OGAR Auto Tous Risques
        'habitation': 'ogar_habitation',  // OGAR Habitation  
        'vie': 'nsia_vie',                // NSIA Vie Protection
        'sante': 'nsia_sante'             // NSIA Santé Plus
      };
    ...
    const applicationData: InsuranceApplicationRequest = {
      insurance_product_id: validProductId, // ✅ Utiliser l'ID valide
      ...
      application_data: {
        quote_reference: this.quoteData?.id,
        insurance_type: insuranceType,
        monthly_premium: this.quoteData?.monthlyPremium,
        annual_premium: this.quoteData?.annualPremium,
        deductible: this.quoteData?.deductible,
        coverage_details: this.quoteData?.coverage_details,
        form_data: formData,
        consent: {
        ...
        original_product_id: this.productData?.id, // Garder l'ID original pour debug
    ...
    this.insuranceApplicationService.submitInsuranceApplication(applicationData).subscribe({
      next: (response: InsuranceApplicationNotification) => {
        ...
        this.notificationService.showSuccess(
          `Votre demande d'assurance a été transmise avec succès !`,
          'Demande envoyée !'
        );
    ...
    this.notificationService.showError('Une erreur inattendue est survenue lors de l\'envoi de votre demande');
    this.isSubmitting = false;
  }
}
  ...
  getCompanyName(): string {
    return this.productData?.insurance_company?.name || 
